...
function renderCards(list) {
    ...
    if (list.length === 0) {
        container.innerHTML = `<p style="text-align:center;color:#A0826D;padding:2rem;font-size:0.95rem;">${traduzirCliente('servicosPag.vazioLimpeza')}</p>`;
        return;
    }
    ...
    list.forEach(pro => {
        ...
        card.innerHTML = `
            ...
            <div class="pro-info">
                ...
                <div class="pro-service">${traduzirCliente('servicosPag.categoriaLimpezaResidencial')}</div>
                <div class="pro-meta">
...
function openModal(pro) {
    ...
    document.getElementById('modalCategoria').textContent = traduzirCliente('servicosPag.categoriaLimpezaResidencial');
...
function abrirModalDetalhes(pro) {
    const idioma = facosClienteIdiomaAtual();
    const categoriaTexto = traduzirCliente('servicosPag.categoriaLimpezaResidencial');
