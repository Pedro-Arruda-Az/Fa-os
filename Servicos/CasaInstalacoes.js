...
async function buscarProfissionais() {
    ...
    const { data, error } = await supabaseClient
        .from('profissionais')
        .select('*')
        .eq('area_atuacao', 'Casa e instalações')
        .eq('status', 'ativo');
    ...
    return (data || []).map((row, index) => {
        ...
        return {
            ...
            services: row.descricao || 'Reformas e instalações residenciais',
...
function renderCards(list) {
    ...
    if (list.length === 0) {
        container.innerHTML = `<p style="text-align:center;color:#A0826D;padding:2rem;font-size:0.95rem;">${traduzirCliente('servicosPag.vazioCasaInstalacoes')}</p>`;
        return;
    }
    ...
    list.forEach(pro => {
        ...
        card.innerHTML = `
            ...
            <div class="pro-info">
                ...
                <div class="pro-service">${traduzirCliente('servico.casaInstalacoes')}</div>
                <div class="pro-meta">
...
function openModal(pro) {
    ...
    document.getElementById('modalCategoria').textContent = traduzirCliente('servico.casaInstalacoes');
...
function abrirModalDetalhes(pro) {
    const idioma = facosClienteIdiomaAtual();
    const categoriaTexto = traduzirCliente('servico.casaInstalacoes');
