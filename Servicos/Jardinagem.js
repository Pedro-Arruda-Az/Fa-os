...
async function buscarProfissionais() {
    ...
    const { data, error } = await supabaseClient
        .from('profissionais')
        .select('*')
        .eq('area_atuacao', 'Jardinagem e áreas externas')
        .eq('status', 'ativo');
    ...
    return (data || []).map((row, index) => {
        ...
        return {
            ...
            email: row.email,
            service: 'Jardinagem e áreas externas',
            ...
            services: row.descricao || 'Jardinagem e cuidado de áreas externas',
...
function renderCards(list) {
    ...
    if (list.length === 0) {
        container.innerHTML = `<p style="text-align:center;color:#A0826D;padding:2rem;font-size:0.95rem;">${traduzirCliente('servicosPag.vazioJardinagem')}</p>`;
        return;
    }
    ...
    list.forEach(pro => {
        ...
        card.innerHTML = `
            ...
            <div class="pro-info">
                ...
                <div class="pro-service">${traduzirCliente('servico.jardinagem')}</div>
                <div class="pro-meta">
...
function openModal(pro) {
    ...
    document.getElementById('modalCategoria').textContent = traduzirCliente('servico.jardinagem');
...
function abrirModalDetalhes(pro) {
    const idioma = facosClienteIdiomaAtual();
    const categoriaTexto = traduzirCliente('servico.jardinagem');
